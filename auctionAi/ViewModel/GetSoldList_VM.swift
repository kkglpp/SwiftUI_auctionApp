...
protocol GetSoldListProtocol{
    func getSoldList(item:(String,[CompletedAuctionModel]))
}

struct GetSoldList_VM{
    var delegate: GetSoldListProtocol!
    let urlPath = "\(HOST)/bidded/user/selling" //Login_Func 파일에서 선언해두었음.
    ...
    //            let auctionData = try decoder.decode(AuctionJsonModel.self, from: basicResult.result) // 만들어놓은 Json모델과 연결
            ...
            DispatchQueue.main.async {
                self.delegate.getSoldList(item: (rsmessage, locations))
                return
            }//dispatchQue
        } catch {
